...
function Section({ title, languages }: { title: string; languages: { name: string; code: string }[] }) {
  return (
    <section>
      {/* Title */}
      <div className="flex items-center gap-4 py-4">
        <span className="h-8 w-1 bg-[#D4AF37]" />
        <h2 className="text-3xl font-semibold text-[#1F3A4A]">{title}</h2>
      </div>

      {/* Grid */}
      <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-5 my-4">
        {languages.map((lang, i) => (
          <div key={i} className="flex justify-center items-center gap-2 rounded-md bg-[#EDEDED] py-1">
            <span className={`fi fi-${lang.code}`}></span>
            <span className="text-md font-medium text-[#1F3A4A]">{lang.name}</span>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <div className="relative py-20">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <Section title="Middle Eastern Languages" languages={middleEasternLanguages} />
      </div>
    </div>
  );
}
